...
exports.createCourse = async (req, res) => {
    ...
    if (await isEmpty(courseName,courseCode,shortDes,fullDes,courseURL))
            ...
            res.send(makeJson("There's already an course with the same code"));
            else
...
exports.updateCourse = async (req,res) => {
    ...
    if (await isEmpty(courseName,courseCode,departments,shortDes,fullDes,courseURL))
            ...
            var check=await CourseDAO.updateCourse(id,courseName,courseCode,departments,shortDes,fullDes,courseURL,teachers);
            //if new code existed in database
            if (check==-1) 
                res.send("New course code already existed");
            else
                if (check==1)
                    res.status(200).send(makeJson("Update successfully"));
                else if (check==0)
                    res.send(makeJson("There was an error"));
